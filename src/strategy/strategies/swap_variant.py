"""Strategy 3: Swap Variant — Role-reversing probe to mitigate mutual blindness."""
...
@register_strategy("swap_variant", parse_swap_variant_config)
class SwapVariantStrategy(SearchStrategy):
    def __init__(
        self,
        *,
        config: SwapVariantConfig,
        w: float,
        k: float,
    ) -> None:
        self.config = config
    ...
    @classmethod
    def from_config(cls, config: ScenarioConfig) -> SwapVariantStrategy:
        params = config.strategy.params.get("swap_variant")
        if params is None:
            params = SwapVariantConfig(
                spiral_radius=0.05,
                lock_duration=1.0,
                s2_radius_mod=1.0,
            )
